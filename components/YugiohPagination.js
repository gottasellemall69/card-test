  ...
  return (
    <>
      <nav aria-label="Pagination Navigation">
        <ul className="mt-2 items-center gap-2 inline-flex mx-auto w-fit border-t border-gray-200 bg-transparent px-4 py-3 sm:px-6">
          {currentPage>1&&(
            <li>
          ...
          {Array.from({length: pageCount},(_,i) => (
            <li className="hidden gap-2 mx-auto w-fit sm:flex flex-row items-center sm:justify-center" key={i+1}>
          ...
          <div className="mx-auto gap-2 flex flex-row items-center sm:space-y-2 sm:justify-center sm:hidden">
            {Array.from({length: currentPage-currentPage+1},(_,i) => i+1).map((page) => (
              <li className={`mx-auto relative w-fit inline-flex text-white text-shadow`} key={page}>
